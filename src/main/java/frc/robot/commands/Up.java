...
public class Up extends Command {
    ...
    public Up(RobotContainer robotContainer, Elevator elevator, Wrist wrist, Claw claw) {
    ...
    public void execute() {
        ...
        if(robotContainer.isCoral){
            ...
            if (Math.abs
                (m_elevator.getPosition() - targetElevatorPos)
        ...
        // ALGAE
            ...
            // for things we need elevator for
                ...
                if (
                    ...
                    m_wrist.setWrist(Constants.WristConstants.pos[RobotState.barge.ordinal()]); // safe position for elev to move up
                } else if (
                    m_elevator.getPosition() >
                    Constants.ElevatorConstants.wristDownSafeThreshold) {
                    m_wrist.setWrist(targetWristPos); // only move wrist down when elevator up (prevents wrist/bumper collision)
                ...
                if (Math.abs
                    (m_elevator.getPosition() - targetElevatorPos)
                    < Constants.ElevatorConstants.posTolerance) {
                        if (targetState.equals(RobotState.algaeReefHigh) 
                        || targetState.equals(RobotState.algaeReefLow)) {
                            m_claw.setClaw(Constants.EndEffectorConstants.velocity);
                        }
                        else if (m_claw.gamepieceDetected() && targetState.equals(RobotState.barge)){
                            m_claw.setClaw(-Constants.EndEffectorConstants.outtakeVelocity);
                        }
                }
                if(!m_claw.gamepieceDetected() && targetState.equals(RobotState.barge)){
                    m_claw.stopClaw();
                }
            }
        }
